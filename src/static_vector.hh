...
struct static_vector {
  ...
  iterator insert(const_iterator pos, const value_type& value) {
    if (full()) return const_cast<iterator>(pos);
  ...
  iterator insert(const_iterator pos, size_type count, const value_type& value) {
    ...
    // Construct value, do not assign nonexistent
    std::for_each(storage_begin() + (mut_pos - begin()),
  ...
  auto insert(const_iterator pos, InputIter insert_begin, InputIter insert_end)
      -> std::enable_if_t<std::is_same<typename std::iterator_traits<InputIter>::reference,
